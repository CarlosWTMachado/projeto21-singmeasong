...
export default function Recommendation({ name, youtubeLink, score, id, onUpvote = () => 0, onDownvote = () => 0 }) {
	...
	return (
		<Container>
			...
			<Row>
				<GoArrowUp size="24px" onClick={handleUpvote} data-test-id="arrow-up" />
				<p data-test-id="score">{score}</p>
				<GoArrowDown size="24px" onClick={handleDownvote} data-test-id="arrow-down" />
			</Row>
		</Container>
	);
}
